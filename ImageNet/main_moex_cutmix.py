...
def train(train_loader, model, criterion, optimizer, epoch):
            ...
            if torch.rand(1).item() < args.moex_prob:
                ...
                if torch.is_tensor(cutmix_lam):
                    loss = (criterion(output, target_a, reduction='none') * cutmix_lam * lam).mean() \
                           + (criterion(output, target_b, reduction='none') * (
                ...
                if torch.is_tensor(cutmix_lam):
                    loss = (criterion(output, target_a, reduction='none') * cutmix_lam).mean() + (
                                criterion(output, target_b, reduction='none') * (1. - cutmix_lam)).mean()
                else:
    ...
    if args.local_rank == 0:
        print(
            f'Train: Prec@1 {top1.avg:.3f} Prec@5 {top5.avg:.3f} Loss {losses.avg:.4f} Time {time.time() - start:.2f}')
