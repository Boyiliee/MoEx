...
parser.add_argument('--lr_scheduler', default='cosine', type=str, choices=['step', 'cosine'])
...
def train(train_loader, model, criterion, optimizer, epoch):
    ...
    while input is not None:
        ...
        if args.beta > 0 and prob < args.cutmix_prob:
            cutmix_lam = np.random.beta(args.beta, args.beta)
            with torch.no_grad():
                ex_index = torch.randperm(input.size()[0]).cuda()
                target_a = target
                target_b = target[ex_index]
                bbx1, bby1, bbx2, bby2 = rand_bbox(input.size(), cutmix_lam)
                input[:, :, bbx1:bbx2, bby1:bby2] = input[ex_index, :, bbx1:bbx2, bby1:bby2]
            ...
            if torch.rand(1).item() < args.moex_prob:
                with torch.no_grad():
                    target_c = target_b[ex_index]
                output = model(input, ex_index=ex_index, moex_epsilon=args.moex_epsilon,
                ...
                if torch.is_tensor(cutmix_lam):
                    loss = (criterion(output, target_a, reduction='none') * cutmix_lam * lam).mean() \
                        + (criterion(output, target_b, reduction='none') * ((1. - cutmix_lam) * lam + cutmix_lam * (1. - lam))).mean() \
                        + (criterion(output, target_c, reduction='none') * (1. - cutmix_lam) * (1 - lam)).mean()
                else:
                    loss = criterion(output, target_a) * cutmix_lam * lam \
                        + criterion(output, target_b) * ((1. - cutmix_lam) * lam + cutmix_lam * (1. - lam)) \
                        + criterion(output, target_c) * (1. - cutmix_lam) * (1 - lam)
                ...
                if torch.is_tensor(cutmix_lam):
                    loss = (criterion(output, target_a, reduction='none') * cutmix_lam).mean() + (criterion(output, target_b, reduction='none') * (1. - cutmix_lam)).mean()
                else:
                    loss = criterion(output, target_a) * cutmix_lam + criterion(output, target_b) * (1. - cutmix_lam)
        else:
            if torch.rand(1).item() < args.moex_prob:
                ex_index = torch.randperm(input.size(0), device=input.device)
                with torch.no_grad():
                    target_a = target
                    target_b = target[ex_index]
                output = model(input, ex_index=ex_index, moex_epsilon=args.moex_epsilon,
